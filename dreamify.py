...
def triangleWave(t, period=100):
    tDivPeriod = t / float(period)
    return abs(2 * (tDivPeriod - math.floor(tDivPeriod + 0.5)))

offset1 = 0
offset2 = 0
for foo in range(0,300):
    addV = foo % 2
    addV2 = math.floor(foo / 64)
    offset1 = int(offset1 + addV + addV2)
    offset2 = int(offset2 + (1 - addV) + addV2)
    ...
    def make_step(net, step_size=1.5, end='inception_4c/output', jitter=32, clip=True):
        ...
        ox, oy = (-32 + offset1, -32 + offset2) # np.random.randint(-jitter, jitter+1, 2)
    ...
    savearray(_, sys.argv[2] + "." + str(foo).zfill(4) + ".png")
